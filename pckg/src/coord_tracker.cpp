...
void track_frame(std::string frame){
    ...
    // Lookup the absolute position of the target frame with respect to the "map" frame
    try{
        absolute_pos = tfBuffer.lookupTransform("map",target_frame, ros::Time(0), ros::Duration(4.0));
    } catch(tf2::TransformException& ex){
        ROS_WARN("%s", ex.what()); // Log a warning message if the transform lookup fails
    }
    ...
    while (((yp<-tolerance || yp>tolerance || zp<-tolerance || zp>tolerance) || (movimiento_x|| movimiento_y|| movimiento_z))&& flag_stop){
       ...
       // Store the position of the target frame relative to the "map" frame before looking up the transform again     
        ...
        try{
        camera_pos = tfBuffer.lookupTransform("map","oakd_color_frame", ros::Time(0), ros::Duration(4.0));}
                catch(tf2::TransformException& ex){
                    ROS_WARN("%s", ex.what());
            }
...
bool trk_coordCB(pckg::track_coord::Request &req,
  ...
  coordTransform.header.frame_id = "map";
